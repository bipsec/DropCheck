...
/**
 * Cookie attributes that differ between local dev and the deployed
 * split, where the browser is on Vercel and this API is on Render.
 *
 * The three attributes move together because the browser rules force it:
 *
 *   - A cross-site cookie must be `SameSite=None`, otherwise the browser
 *     simply won't send it to a different registrable domain.
 *   - `SameSite=None` is **rejected outright without `Secure`**.
 *   - `Secure` cookies are refused over plain HTTP (localhost excepted),
 *     which is why dev has to stay on `Lax` rather than just always
 *     using the production values.
 *
 * `Partitioned` is CHIPS: it keeps Chrome sending the cookie after
 * third-party-cookie deprecation. It does not defeat Safari's ITP —
 * Safari users may not hold a session at all until the frontend and API
 * share a registrable domain (or a bearer-token fallback lands).
 */
function sessionCookieAttrs(): {
  ...
  return getSettings().cookie_cross_site
 ...
 *
 * The attributes must match `buildSessionSetCookie` exactly — a browser
 * treats a cookie whose `Secure`/`SameSite`/`Partitioned` differ as a
 * *different* cookie, so a mismatch here silently fails to clear it.
 */
